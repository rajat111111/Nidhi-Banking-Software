...
import { useSubmitSavingAccountDocsMutation } from "../../../../features/api/savingAccounts";
...
const Document = () => {
  ...
  const validationSchema = Yup.object({
    ...
    drivingLicense: Yup.mixed()
      ...
      .test("fileType", "Only JPG,SVG, PNG, PDF allowed", (file) =>
        file ? SUPPORTED_DOC_TYPES.includes(file.type) : true
      ),
    ...
    signature: Yup.mixed()
      ...
      .test("fileType", "Only JPG,SVG, PNG allowed", (file) =>
  ...
  const handleSubmit = async (values) => {
    try {
      ...
      await submitSavingAccountDocs({ id: id, values: formData });
    } catch (err) {
      console.log(err);
    }
  };
